...
def main(input_folder, output_folder):
    ...
    image_paths = sorted(glob.glob(os.path.join(input_folder, 'test_ex.jpg')))
    if not image_paths:
    ...
    warped_path = os.path.join(output_folder, 'warped_result.jpg')
    ...
    denoised_warped = cv2.fastNlMeansDenoising(warped_gray, None, 10, 7, 21)

    # 2. 이미지 확대 (OCR 정확도 향상)
    upscaled_base = cv2.resize(denoised_base, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
    upscaled_warped = cv2.resize(denoised_warped, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
    ...
    binarized_warped = cv2.adaptiveThreshold(upscaled_warped, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 15, 4)
    ...
    try:
        ...
        final_text = pytesseract.image_to_string(binarized_warped, lang='kor+eng', config=custom_config)
